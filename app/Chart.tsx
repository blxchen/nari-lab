...
export default function StormChart({ storm, metric }: { storm: Storm; metric: 'wind'|'pressure'|'rain' }) {
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0,0,w,h); const bg=ctx.createLinearGradient(0,0,0,h);bg.addColorStop(0,'#003054');bg.addColorStop(1,'#001523');ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);ctx.font='9px monospace'; ctx.fillStyle='#89aebc'; ctx.strokeStyle='rgba(116,201,232,.14)'; ctx.lineWidth=1;
    ...
    ctx.fillStyle='#74c9e8';ctx.font='700 9px monospace';ctx.fillText(`${storm.name} · ${metric.toUpperCase()} · NARI LAB`,p.l,p.t-13);const grad=ctx.createLinearGradient(0,p.t,0,h-p.b);grad.addColorStop(0,`${storm.color}aa`);grad.addColorStop(1,`${storm.color}00`);
